...
| Outreach/message packs and status | `validation/private/today-outreach-block.*`, `today-message-pack.*`, `today-next-draft.md`, `today-send-copy.txt`, `today-outreach-status.*`, `send-copy-YYYY-MM-DD/`, `contact-form-copy-YYYY-MM-DD/` | Private validation | Target labels, segment, persona, safe draft text, tracker commands, send/update state, mismatch fields. `today-send-copy.txt`, numbered send-copy files, and numbered contact-form copy files intentionally emit one subject line plus the message body and omit target labels, tracker commands, alternate subject options, and status metadata. The single-file send artifact omits target labels, tracker commands, alternate subject options, and private metadata when the dashboard reports matching ready states for the next draft and send-copy file; aggregate updates may mention send-copy readiness without exposing text. Their manifests, checklists, indexes, READMEs, and DO_NOT_SEND guards are private operator metadata. | Ignored `validation/private/`. | Delete or rotate after the daily outreach block is complete. | Export aggregate counts only; use numbered or single send-copy text only as outbound copy text after the verified dry-run passes and the relevant copy checker reports the files are safe. |
