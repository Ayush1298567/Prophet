...
| `python3 scripts/check-buyer-follow-up-package.py` | Verifies the qualified-buyer follow-up package after a smoke run: tracked buyer docs exist, generated evidence/handoff artifacts exist under ignored runtime paths, runtime files stay untracked, hashes match `scripts/pilot-demo-smoke.sha256`, evidence safety/redaction attestations are present, the handoff manifest is `review_template_only`, customer placeholder mapping is required, and the review ZIP is valid. It does not send outreach, mutate runtime files, open the build gate, or treat runtime artifacts as release-bound commit content. |
