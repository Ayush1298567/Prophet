...
| `python3 scripts/validation-next-action.py` | Renders a private ignored `NEXT_ACTION.md` handoff from the current dashboard; `--date` rejects stale packs, `--out` writes the ignored handoff, records the local git head/worktree state, best-effort GitHub CI status, the single-target pre-send command, and the full-batch pre-send command for restored-session context, and the helper does not send outreach, prune artifacts, confirm writes, or mutate trackers/logs. |
